/**
 * \file GameDevUIViewAnimation3DNetworks.cpp
...
#include "GameDevUIViewAnimation3DNetworks.h"
...
SEOUL_BEGIN_TYPE(Game::DevUIViewAnimation3DNetworks, TypeFlags::kDisableCopy)
	SEOUL_ATTRIBUTE(DisplayName, "Animation3D Networks")
...
DevUIViewAnimation3DNetworks::DevUIViewAnimation3DNetworks()
	: m_sSelected()
	, m_Trigger()
{
}
...
static void AnimState3D(Byte const* sName, const SharedPtr<Animation::NodeInstance>& p)
{
	using namespace Animation;
	using namespace Animation3D;
	...
	case NodeType::kBlend:
		{
			...
			if (TreeNodeEx(sName, ImGuiTreeNodeFlags_DefaultOpen, "%s (Blend)", sName))
			{
				...
				// Children.
				AnimState3D("ChildA", pBlend->GetChildA());
				AnimState3D("ChildB", pBlend->GetChildB());
	...
	case NodeType::kPlayClip:
		{
			SharedPtr<Animation3D::PlayClipInstance> pPlayClip((Animation3D::PlayClipInstance*)p.GetPtr());
	...
	case NodeType::kStateMachine:
		{
			...
			if (TreeNodeEx(sName, ImGuiTreeNodeFlags_DefaultOpen, "%s (StateMachine)", sName))
			{
				...
				{
					AnimState3D(pStateMachine->GetNewId().CStr(), pStateMachine->GetNew());
				}
				if (!pStateMachine->GetOldId().IsEmpty())
				{
					ImGui::PushStyleColor(ImGuiCol_Text, ImGui::GetStyle().Colors[ImGuiCol_TextDisabled]);
					AnimState3D(pStateMachine->GetOldId().CStr(), pStateMachine->GetOld());
					ImGui::PopStyleColor();
				}
...
static String GetNetworkId(const SharedPtr<Animation3D::NetworkInstance>& p)
{
	return String::Printf("%s(%p)",
		p->GetNetworkHandle().GetKey().GetRelativeFilenameWithoutExtension().CStr(),
		p.GetPtr());
}

static Int32 GetSelectedIndex(const String& sSelected, const Animation3D::Manager::Instances& vInstances)
{
...
void DevUIViewAnimation3DNetworks::DoPrePose(DevUI::Controller& rController, RenderPass& rPass)
{
	using namespace Animation3D;
	using namespace ImGui;
	...
	if (iSelected >= 0 && vInstances[iSelected]->IsReady())
	{
		...
		{
			AnimState3D("Root", p->GetRoot());
		}
	}
}

UInt32 DevUIViewAnimation3DNetworks::GetFlags() const
{
