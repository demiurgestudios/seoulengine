...
typedef struct GCudata {
  ...
  // EXT_DEMIURGE: uint32_t align1;	/* To force 8 byte alignment of the payload. */
  uint32_t userprivatedata; /* EXT_DEMIURGE: */
} GCudata;
...
typedef struct global_State {
  ...
  lua_PreFreeUdFunction udmemf; /* EXT_DEMIURGE: Called when user-data memory is about to be freed. */
  lua_PreCollectWeakUd udweakcf; /* EXT_DEMIURGE: Called when a weak ref to a user-data is about to be cleared. */
} global_State;
