...
# define luaL_tolstring(L, idx, len) lua_tolstring((L), (idx), (len)) /* EXT_DEMIURGE: */
#ifndef luaL_newlib /* EXT_DEMIURGE: */
...
static int Lconv_encode_int32(lua_State *L) {
    lua_pushinteger(L, /*EXT_DEMIURGE:*/(lua_Integer)/*EXT_DEMIURGE:*/pb_expandsig((uint32_t)luaL_checkinteger(L, 1)));
    return 1;
}
...
static int Lconv_encode_sint64(lua_State *L) {
    lua_pushinteger(L, /*EXT_DEMIURGE:*/(lua_Integer)/*EXT_DEMIURGE:*/pb_encode_sint64(luaL_checkinteger(L, 1)));
    return 1;
}

static int Lconv_decode_sint64(lua_State *L) {
    lua_pushinteger(L, /*EXT_DEMIURGE:*/(lua_Integer)/*EXT_DEMIURGE:*/pb_decode_sint64(luaL_checkinteger(L, 1)));
    return 1;
}
...
static int Lconv_encode_double(lua_State *L) {
    lua_pushinteger(L, /*EXT_DEMIURGE:*/(lua_Integer)/*EXT_DEMIURGE:*/pb_encode_double(luaL_checknumber(L, 1)));
    return 1;
}
...
static int Lbuf_pair(lua_State *L) {
    ...
    if (!isint) { /* EXT_DEMIURGE: */
        wiretype = find_wiretype(luaL_checkstring(L, 3));
        if (wiretype < 0) { /* EXT_DEMIURGE: */
...
static int Lbuf_add(lua_State *L) {
    ...
    union { float f; uint32_t u32;
            double d; uint64_t u64; /* EXT_DEMIURGE: */size_t z;/* EXT_DEMIURGE: */ } u;
    ...
    case PB_Tstring:
    case PB_Tmessage:
        s = luaL_checklstring(L, 4, &u.z);
        if (haspair) pb_addpair(buf, tag, PB_TDATA);
        pb_addvarint(buf, u.z);
        pb_prepbuffsize(buf, u.z);
        memcpy(&buf->buff[buf->used], s, u.z);
        buf->used += (unsigned)u.z;
        break;
    case PB_Tdouble:
...
static int Lbuf_clear(lua_State *L) {
    ...
    buf->used -= /* EXT_DEMIURGE: */(unsigned)/* EXT_DEMIURGE: */sz;
...
#undef  ENTRY
    ...
    if (luaL_newmetatable(L, PB_BUFFER)) {
        ...
        /*EXT_DEMIURGE: lua_pushvalue(L, -1); */
    }
    return 1;
}
...
static int pb_pushvarint(lua_State *L, pb_Slice *dec, int type) {
    ...
    switch (type) {
    case PB_Tint32: out = /*EXT_DEMIURGE:*/(lua_Integer)/*EXT_DEMIURGE:*/pb_expandsig((uint32_t)n); break;
    case PB_Tuint32:
    ...
    case PB_Tsint32:
        ...
        break;
    case PB_Tsint64:
        out = /*EXT_DEMIURGE:*/(lua_Integer)/*EXT_DEMIURGE:*/pb_decode_sint64(n);
        break;
    case 0:
    case PB_Tint64:
    case PB_Tuint64:
...
static int pb_pushfixed64(lua_State *L, pb_Slice *dec, int type) {
    ...
    case PB_Tsfixed64:
		...
		/* EXT_DEMIURGE: */lua_pushinteger(L, out);/* EXT_DEMIURGE: */
        return 1;
    default:
        dec->p = p;
        return type_mismatch(L, type, "fixed64");
    }
    /* EXT_DEMIURGE: lua_pushinteger(L, out);
    return 1; */
}
...
#endif
        if (!pb_readvarint(dec, &n)) return 0;
        if (dec->end - dec->p < /* EXT_DEMIURGE: */(ptrdiff_t)/* EXT_DEMIURGE: */n) return 0;
...
static void init_decoder(pb_Slice *dec, lua_State *L, int idx) {
    ...
    dec->p   = s.p + i - 1;
    dec->end = s.p + j;
	lua_pushlightuserdata(L, dec); /* EXT_DEMIURGE: */
    lua_pushvalue(L, idx);
    lua_rawset(L, LUA_REGISTRYINDEX);  /* EXT_DEMIURGE: lua_rawsetp(L, LUA_REGISTRYINDEX, dec); */
}
...
static int Lslice_source(lua_State *L) {
	...
	lua_pushlightuserdata(L, dec); /* EXT_DEMIURGE: */
    lua_rawget(L, LUA_REGISTRYINDEX); /* EXT_DEMIURGE: lua_rawgetp(L, LUA_REGISTRYINDEX, dec); */
...
static int Lslice_bytes(lua_State *L) {
    ...
    if (n == 0 && !pb_readvarint(dec, &n))
        return 0;
    if (dec->end - dec->p < /* EXT_DEMIURGE: */(ptrdiff_t)/* EXT_DEMIURGE: */n) {
        dec->p = p;
        return 0;
    }
    lua_pushlstring(L, dec->p, (size_t)n);
