...
#pragma warning( suppress : 6001 )
    ...
    /* EXT_DEMIURGE: uint32_t i, j, */delta = pType->NumericType.IsPackedArray ? 1 : SType::c_ScalarsPerRegister;
    pDestValues += Offset * delta;
    for (/* EXT_DEMIURGE: */uint32_t /* EXT_DEMIURGE: */i = 0, j = 0; j < Count; i += delta, ++ j)
    {
...
template<typename IBaseInterface> struct TGlobalVariable; // EXT_DEMIURGE:
template<typename IBaseInterface> struct TTopLevelVariable; // EXT_DEMIURGE:

// EXT_DEMIURGE BEGIN:
#define COMMON_USINGS \
    using Super::Data; \
    using Super::ExplicitBindPoint; \
    using Super::pMemberData; \
    using Super::pName; \
    using Super::pSemantic; \
    using Super::pType; \
    using Super::GetEffect; \
    using Super::GetTopLevelEntity; \
    using Super::GetTotalUnpackedSize; \
    using Super::IsArray;
#define NUMERIC_USINGS \
    COMMON_USINGS \
    using Super::DirtyVariable; \
// EXT_DEMIURGE END:
...
{
    typedef IBaseInterface Super; COMMON_USINGS // EXT_DEMIURGE:
...
{
    typedef TVariable<TTopLevelVariable<IBaseInterface> > Super; COMMON_USINGS // EXT_DEMIURGE:
...
{
    typedef TVariable<TTopLevelVariable<IBaseInterface> > Super; COMMON_USINGS // EXT_DEMIURGE:
    using Super::pEffect; // EXT_DEMIURGE:
...
{
    typedef IBaseInterface Super; NUMERIC_USINGS // EXT_DEMIURGE:
...
{
    typedef TNumericVariable<IBaseInterface, IsAnnotation> Super; NUMERIC_USINGS // EXT_DEMIURGE:
...
{
    typedef TVectorVariable<IBaseInterface, false, ETVT_Float> Super; NUMERIC_USINGS // EXT_DEMIURGE:
...
{
    typedef TMatrixVariable<IBaseInterface, false> Super; NUMERIC_USINGS // EXT_DEMIURGE:
...
#define CHECK_OBJECT_ARRAY_BOUNDS(Offset, Count, Pointer) \
    ...
    elements = IsArray() ? pType->Elements : 1; \
    \
...
#undef NUMERIC_USINGS // EXT_DEMIURGE:
#undef COMMON_USINGS // EXT_DEMIURGE:
