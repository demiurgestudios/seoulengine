...
{
    int result = -1; // EXT_DEMIURGE:
    const CHAR* pstrName = nullptr; // EXT_DEMIURGE:
    ...
    /* EXT_DEMIURGE:int */result = WideCharToMultiByte(CP_ACP, WC_NO_BEST_FIT_CHARS, pFileName, -1, strFileA, MAX_PATH, nullptr, FALSE);
    if ( !result )
    {
    ...
    /* EXT_DEMIURGE:const CHAR* */pstrName = strrchr(strFileA, '\\');
    if (!pstrName)
    {
        pstrName = strFileA;
    }
