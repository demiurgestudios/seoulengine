...
CEffect::CEffect( uint32_t Flags ) noexcept :
	...
	m_pActiveTechnique(nullptr), // EXT_DEMIURGE:
    m_pTypePool(nullptr),
...
CEffect::~CEffect()
{
    ...
    if (pInfoQueue)
    {
        ...
        // EXT_DEMIURGE:
        D3D11_MESSAGE_CATEGORY messageCategory[] = {
            D3D11_MESSAGE_CATEGORY_STATE_SETTING,
            D3D11_MESSAGE_CATEGORY_STATE_CREATION,
        };
        // /EXT_DEMIURGE:
        ZeroMemory(&filter, sizeof(filter)); // EXT_DEMIURGE:

        filter.DenyList.NumCategories = 2 /* EXT_DEMIURGE: 1*/;
        filter.DenyList.pCategoryList = &messageCategory[0];
...
{
    HRESULT hr = S_OK;
    uint32_t CurMemberData = 0; // EXT_DEMIURGE:
    ID3D11ClassLinkage* neededClassLinkage = nullptr; // EXT_DEMIURGE:
    SShaderBlock* pShader = nullptr; // EXT_DEMIURGE:
    SShaderBlock* pShaderLast = nullptr; // EXT_DEMIURGE:
    SConstantBuffer* pCB = nullptr; // EXT_DEMIURGE:
    SConstantBuffer* pCBLast = nullptr; // EXT_DEMIURGE:
    SRasterizerBlock* pRB = nullptr; // EXT_DEMIURGE:
    SRasterizerBlock* pRBLast = nullptr; // EXT_DEMIURGE:
    SDepthStencilBlock* pDS = nullptr; // EXT_DEMIURGE:
    SDepthStencilBlock* pDSLast = nullptr; // EXT_DEMIURGE:
    SBlendBlock* pBlend = nullptr; // EXT_DEMIURGE:
    SBlendBlock* pBlendLast = nullptr; // EXT_DEMIURGE:
    SSamplerBlock* pSampler = nullptr; // EXT_DEMIURGE:
    SSamplerBlock* pSamplerLast = nullptr; // EXT_DEMIURGE:
    ...
    /* EXT_DEMIURGE: SConstantBuffer* */pCB = m_pCBs;
    /* EXT_DEMIURGE: SConstantBuffer* */pCBLast = m_pCBs + m_CBCount;
    for(; pCB != pCBLast; pCB++)
    {
    ...
    /* EXT_DEMIURGE: SRasterizerBlock* */pRB = m_pRasterizerBlocks;
    /* EXT_DEMIURGE: SRasterizerBlock* */pRBLast = m_pRasterizerBlocks + m_RasterizerBlockCount;
    for(; pRB != pRBLast; pRB++)
    {
    ...
    /* EXT_DEMIURGE: SDepthStencilBlock* */pDS = m_pDepthStencilBlocks;
    /* EXT_DEMIURGE: SDepthStencilBlock* */pDSLast = m_pDepthStencilBlocks + m_DepthStencilBlockCount;
    for(; pDS != pDSLast; pDS++)
    {
    ...
    /* EXT_DEMIURGE: SBlendBlock* */pBlend = m_pBlendBlocks;
    /* EXT_DEMIURGE: SBlendBlock* */pBlendLast = m_pBlendBlocks + m_BlendBlockCount;
    ...
    // Create all Samplers
    /* EXT_DEMIURGE: SSamplerBlock* */pSampler = m_pSamplerBlocks;
    /* EXT_DEMIURGE: SSamplerBlock* */pSamplerLast = m_pSamplerBlocks + m_SamplerBlockCount;
    ...
    // Create all shaders
    /* EXT_DEMIURGE: ID3D11ClassLinkage* */neededClassLinkage = featureLevelGE11 ? m_pClassLinkage : nullptr;
    /* EXT_DEMIURGE: SShaderBlock* */pShader = m_pShaderBlocks;
    /* EXT_DEMIURGE: SShaderBlock* */pShaderLast = m_pShaderBlocks + m_ShaderBlockCount;
    ...
    /* EXT_DEMIURGE: uint32_t */CurMemberData = 0;
...
HRESULT CEffect::CopyOptimizedTypePool( CEffect* pEffectSource, CPointerMappingTable& mappingTableTypes )
{
    ...
    uint8_t* pReadTypes = nullptr; /* EXT_DEMIURGE: */
    ...
    // first pass: move types over, build mapping table
    /* EXT_DEMIURGE: uint8_t* */pReadTypes = pEffectSource->m_pOptimizedTypeHeap->GetDataStart();
...
{
    HRESULT hr = S_OK;
    uint32_t Members = 0; // EXT_DEMIURGE:
    ...
    /* EXT_DEMIURGE: uint32_t */Members = m_pMemberInterfaces.GetSize();
