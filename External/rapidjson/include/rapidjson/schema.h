...
public:
    ...
    virtual void* ReallocState(void* p, size_t oldSize, size_t newSize) = 0; // EXT_DEMIURGE:
...
public:
    ...
    void Comment(size_t offsetBegin, size_t offsetEnd) {} /* EXT_DEMIURGE: */
...
public:
    ...
    bool StartObject(Context& context) const {
        ...
        if (patternProperties_) { // pre-allocate schema array
            ...
            if (context.patternPropertiesSchemaCount != count) { // EXT_DEMIURGE:
                // EXT_DEMIURGE: context.patternPropertiesSchemas = static_cast<const SchemaType**>(context.factory.MallocState(sizeof(const SchemaType*) * count));
                context.patternPropertiesSchemas = static_cast<const SchemaType**>(context.factory.ReallocState(context.patternPropertiesSchemas, sizeof(const SchemaType*) * context.patternPropertiesSchemaCount, sizeof(const SchemaType*) * count)); // EXT_DEMIURGE:
			} // EXT_DEMIURGE:
    ...
    bool Key(Context& context, const Ch* str, SizeType len, bool) const {
        ...
        // EXT_DEMIURGE:
		else if (context.patternPropertiesSchemaCount > 0) {
            if (context.patternPropertiesSchemaCount == 1) {
				context.valueSchema = context.patternPropertiesSchemas[0];
			}
			else {
				context.valueSchema = typeless_;
			}
...
#undef RAPIDJSON_SCHEMA_HANDLE_VALUE_
    ...
    // EXT_DEMIURGE:
    virtual void* ReallocState(void* p, size_t oldSize, size_t newSize) {
        return GetStateAllocator().Realloc(p, oldSize, newSize);
	}
    // /EXT_DEMIURGE:
    ...
    bool BeginValue() {
        if (schemaStack_.Empty())
            ...
            if (count > 0) {
                ...
                // EXT_DEMIURGE: va = static_cast<ISchemaValidator**>(MallocState(sizeof(ISchemaValidator*) * count));
                va = static_cast<ISchemaValidator**>(ReallocState(va, sizeof(ISchemaValidator*) * validatorCount, sizeof(ISchemaValidator*) * (validatorCount + count))); // EXT_DEMIURGE:
                for (SizeType i = 0; i < count; i++)
