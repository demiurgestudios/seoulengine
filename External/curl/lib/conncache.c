...
void Curl_conncache_close_all_connections(struct conncache *connc)
{
  ...
  if(connc->closure_handle) {
    ...
    Curl_close(connc->closure_handle);
    connc->closure_handle = NULL; /* EXT_DEMIURGE: */
    sigpipe_restore(&pipe_st);
  }
}

/* EXT_DEMIURGE: BEGIN DEMI SPECIFIC, HACKY - we exploit knowledge about our share implementation and assume it's already been locked. */
void Curl_demiurge_share_inside_lock_prune_idle_connections(struct Curl_share *share, long timeout_ms)
{
  struct curl_hash_iterator iter;
  struct curl_llist_element *curr;
  struct curl_hash_element *he;
  struct conncache *connc = &share->conn_cache;
  ...
  while (he) {
    ...
    curr = bundle->conn_list.head;
    while (curr) {
     /* Update before check since we might remove the connection. */
     struct connectdata *conn = curr->ptr;
     curr = curr->next;
     ...
     if (Curl_timediff(now, conn->receivedat) >= timeout_ms) {
       /* Use closure easy for processing. */
       SIGPIPE_VARIABLE(pipe_st);
       conn->data = connc->closure_handle;
