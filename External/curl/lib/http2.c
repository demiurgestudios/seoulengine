  ...
  if(sval == 0) {
    ...
    if(!dead) {
      ...
      if(nread != -1) {
        ...
        if (nread > 0) /* EXT_DEMIURGE: */
          conn->receivedat = Curl_now(); /* EXT_DEMIURGE: */
      }
      else
                                    ...
                                    CURLcode *err)
{
  ...
  if(should_close_session(httpc)) {
    // EXT_DEMIURGE: Extra error context
    H2BUGF(infof(data,
                 "h2_process_pending_input: nothing to do in this session: %s\n", Curl_http2_strerror(httpc->error_code)));
    /*
     * EXT_DEMIURGE: BEGIN CHANGES - Based on comments elsewhere and in the nghttp2 release notes
     * (e.g. https://nghttp2.org/blog/2017/12/19/nghttp2-v1-29-0/), NGHTTP2_REFUSED_STREAM is a
     * clean closure and should be treated as GOAWAY.
     */
    if(NGHTTP2_REFUSED_STREAM == httpc->error_code) {
      connclose(conn, "REFUSED_STREAM"); /* reason */
      data->state.refused_stream = TRUE; /* mark for retry */
      *err = CURLE_RECV_ERROR; /* also mark for retry */
    }
    else
    /* EXT_DEMIURGE: END CHANGES */
    if(httpc->error_code)
      *err = CURLE_HTTP2;
    else {
                          ...
                          char *mem, size_t len, CURLcode *err)
{
  ...
  if(should_close_session(httpc)) {
    // EXT_DEMIURGE: Extra error context
    H2BUGF(infof(data,
                 "http2_recv(pre-check): nothing to do in this session: %s\n", Curl_http2_strerror(httpc->error_code)));
    *err = CURLE_HTTP2;
    return -1;
  }
  ...
  // EXT_DEMIURGE: Extra status context
  H2BUGF(infof(data, "http2_recv: easy %p (stream %d, connection_id=%ld, local_port=%ld len=%u)\n",
               data, stream->stream_id, conn->connection_id, conn->local_port, (unsigned int)len));
  ...
  if((data->state.drain) && stream->memlen) {
    ...
    *err = CURLE_AGAIN;
    ...
    if(httpc->inbuflen == 0) {
      ...
      if(nread == 0) {
        // EXT_DEMIURGE: Extra status context
        H2BUGF(infof(data, "end of stream %d\n", stream->stream_id));
        *err = CURLE_OK;
        return 0;
      }
    ...
    if(should_close_session(httpc)) {
      // EXT_DEMIURGE: Extra error context
      H2BUGF(infof(data, "http2_recv(after h2_session_send): nothing to do in this session: %s\n", Curl_http2_strerror(httpc->error_code)));
      *err = CURLE_HTTP2;
      return -1;
    }
  }
                          ...
                          const void *mem, size_t len, CURLcode *err)
{
  ...
  // EXT_DEMIURGE: Extra status context
  H2BUGF(infof(conn->data, "http2_send len=%zu stream_id=%d connection_id=%ld, local_port=%ld\n", len, stream->stream_id, conn->connection_id, conn->local_port));
  ...
  if(stream->stream_id != -1) {
    ...
    if(should_close_session(httpc)) {
      // EXT_DEMIURGE: Extra error context
      H2BUGF(infof(conn->data, "http2_send: nothing to do in this session: %s\n", Curl_http2_strerror(httpc->error_code)));
      *err = CURLE_HTTP2;
      return -1;
    }
                             ...
                             const char *mem, size_t nread)
{
  ...
  if(should_close_session(httpc)) {
    // EXT_DEMIURGE: Extra error context
    H2BUGF(infof(data,
                 "nghttp2_session_send(): nothing to do in this session: %s\n", Curl_http2_strerror(httpc->error_code)));
    return CURLE_HTTP2;
  }
