...
b3Shape* b3Body::CreateShape(const b3ShapeDef& def) 
{
	...
	// Mark this body as having a sensor.
	if (def.isSensor)
	{
		m_flags |= e_hasSensor;
	}
...
{
	// Decide if we need to recheck the sensor flag.
	bool recheckSensor = shape->IsSensor();

	// Remove the shape from this body shape list.
	B3_ASSERT(shape->m_body == this);
	m_shapeList.Remove(shape);

	// Now decide if we're still a sensor or not.
	if (recheckSensor)
	{
		bool hasSensor = false;
		for (auto s = m_shapeList.m_head; s; s = s->m_next)
		{
			if (s->IsSensor())
			{
				hasSensor = true;
		...
		// Kill the sensor flag if we are no longer a sensor.
		if (!hasSensor)
		{
			m_flags &= ~e_hasSensor;
		}
...
void b3Body::DestroyShapes() 
{
	...
	// No longer a sensor with no shapes.
	m_flags &= ~e_hasSensor;
}
...
bool b3Body::ShouldCollide(const b3Body* other) const
{
	...
	{
		// Also allow collisions if one of the two bodies has
		// a sensor and the other is kinematic (since we know
		// it can't be dynamic, and we don't want to collision
		// with static.
		if ((!HasSensor() || other->m_type != e_kinematicBody) &&
			(m_type != e_kinematicBody || !other->HasSensor()))
		{
