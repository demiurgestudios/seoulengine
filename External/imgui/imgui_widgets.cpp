...
void ImGui::TextEx(const char* text, const char* text_end, ImGuiTextFlags flags/* EXT_DEMIURGE: */, bool with_id /*= false */ /* EXT_DEMIURGE: */)
{
    ...
    if (text_end - text <= 2000 || wrap_enabled)
    {
        ...
        ImGuiID id = (with_id ? window->GetID(text_begin, text_end) : 0); // EXT_DEMIURGE:
        if (!ItemAdd(bb, /* EXT_DEMIURGE: 0*/id/* EXT_DEMIURGE: */))
            return;
        if (with_id) // EXT_DEMIURGE:
            ItemHoverable(bb, id); // EXT_DEMIURGE: Calls SetHoveredID(id) on success.
...
void ImGui::TextUnformatted(const char* text, const char* text_end/* EXT_DEMIURGE: */, bool with_id /*= false */ /* EXT_DEMIURGE: */)
{
    TextEx(text, text_end, ImGuiTextFlags_NoWidthForLargeClippedText/* EXT_DEMIURGE: */, with_id/* EXT_DEMIURGE: */);
}
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags/* EXT_DEMIURGE: */, bool indeterminate/* EXT_DEMIURGE: */)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (indeterminate) { // EXT_DEMIURGE:
        const ImVec2 center = bb.GetCenter(); // EXT_DEMIURGE:
        const float fLeft = bb.Min.x + style.FramePadding.x; // EXT_DEMIURGE:
        const float fRight = ImMin( // EXT_DEMIURGE:
            bb.Max.x - style.FramePadding.x, // EXT_DEMIURGE:
            bb.Min.x + style.FramePadding.x + g.FontSize); // EXT_DEMIURGE:

        window->DrawList->AddLine( // EXT_DEMIURGE:
            ImVec2(fLeft, center.y), ImVec2(fRight, center.y), GetColorU32(ImGuiCol_Text)); // EXT_DEMIURGE:
    } else { // EXT_DEMIURGE:
...
bool ImGui::Button(const char* label, const ImVec2& size_arg/* EXT_DEMIURGE: */, bool enabled /* = true */, bool indeterminate /* = false */ /* EXT_DEMIURGE: */)
{
    if (!enabled) BeginDisabled();
    /* EXT_DEMIURGE: return */bool ret = /* EXT_DEMIURGE: */ButtonEx(label, size_arg, ImGuiButtonFlags_None /* EXT_DEMIURGE: */, indeterminate /* EXT_DEMIURGE: */);
    if (!enabled) EndDisabled();
    return ret; // EXT_DEMIURGE:
}
...
bool ImGui::ImageButtonEx(ImGuiID id, ImTextureID texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec2& padding, const ImVec4& bg_col, const ImVec4& tint_col/* EXT_DEMIURGE: */, bool enabled /*= true */ /* EXT_DEMIURGE: */)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
    ...
    if (!enabled) BeginDisabled(); // EXT_DEMIURGE:
    bool hovered, held;
    ...
    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    RenderNavHighlight(bb, id);
    /* EXT_DEMIURGE: */ if (hovered || bg_col.w > 0.0f) /* EXT_DEMIURGE: */ RenderFrame(bb.Min, bb.Max, col, true, ImClamp((float)ImMin(padding.x, padding.y), 0.0f, g.Style.FrameRounding));
    if (bg_col.w > 0.0f)
        window->DrawList->AddRectFilled(bb.Min + padding, bb.Max - padding, GetColorU32(bg_col));
    window->DrawList->AddImage(texture_id, bb.Min + padding, bb.Max - padding, uv0, uv1, GetColorU32(tint_col));

    if (!enabled) EndDisabled(); // EXT_DEMIURGE:
    return pressed;
}
...
bool ImGui::ImageButton(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col/* EXT_DEMIURGE: */, bool enabled /*= true*/ /* EXT_DEMIURGE: */)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems)
    ...
    return ImageButtonEx(id, user_texture_id, size, uv0, uv1, padding, bg_col, tint_col/* EXT_DEMIURGE: */, enabled/* EXT_DEMIURGE: */);
}

bool ImGui::Checkbox(const char* label, bool* v/* EXT_DEMIURGE: */, bool enabled /*= true*/, bool indeterminate /*= false*/ /* EXT_DEMIURGE: */)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (/* EXT_DEMIURGE: */enabled && /* EXT_DEMIURGE: */pressed)
    {
        *v = !(*v);
        MarkItemEdited(id);
    }
    ...
    // EXT_DEMIURGE: ImU32 check_col = GetColorU32(ImGuiCol_CheckMark);
	ImU32 check_col = GetColorU32(enabled ? ImGuiCol_CheckMark : ImGuiCol_TextDisabled); // EXT_DEMIURGE:
    bool mixed_value = (/* EXT_DEMIURGE: */indeterminate || (/*EXT_DEMIURGE: */g.LastItemData.InFlags & ImGuiItemFlags_MixedValue) != 0/* EXT_DEMIURGE: */)/* EXT_DEMIURGE: */;
    if (mixed_value)
    {
...
template<typename T>
bool ImGui::CheckboxFlagsT(const char* label, T* flags, T flags_value/* EXT_DEMIURGE: */, bool enabled /* = true */, unsigned int indeterminate /* = 0 */ /* EXT_DEMIURGE: */)
{
    ...
    if (!all_on && any_on)
    {
        ...
        pressed = Checkbox(label, &all_on/* EXT_DEMIURGE: */, enabled, (indeterminate & flags_value) == flags_value/* EXT_DEMIURGE: */);
...
bool ImGui::CheckboxFlags(const char* label, int* flags, int flags_value/* EXT_DEMIURGE: */, bool enabled /* = true */, unsigned int indeterminate /* = 0 */ /* EXT_DEMIURGE: */)
{
    return CheckboxFlagsT(label, flags, flags_value/* EXT_DEMIURGE: */, enabled, indeterminate/* EXT_DEMIURGE: */);
}

bool ImGui::CheckboxFlags(const char* label, unsigned int* flags, unsigned int flags_value/* EXT_DEMIURGE: */, bool enabled /* = true */, unsigned int indeterminate /* = 0 */ /* EXT_DEMIURGE: */)
{
    return CheckboxFlagsT(label, flags, flags_value/* EXT_DEMIURGE: */, enabled, indeterminate/* EXT_DEMIURGE: */);
}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags/* EXT_DEMIURGE: */, bool enabled, bool indeterminate /* EXT_DEMIURGE: */)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    ...
    {
        ImVec2 preview_pos = bb.Min + style.FramePadding; // EXT_DEMIURGE:
        if (g.LogEnabled)
            LogSetNextTextDecoration("{", "}");
        ...
        if (indeterminate) { // EXT_DEMIURGE:
            ...
            RenderTextClipped(/* EXT_DEMIURGE: bb.Min + style.FramePadding*/preview_pos/* EXT_DEMIURGE: */, ImVec2(value_x2, bb.Max.y), preview_value, NULL, NULL);
        } // EXT_DEMIURGE:
    }
    if (label_size.x > 0)
    ...
    if (!enabled) EndDisabled(); // EXT_DEMIURGE:
    if (!popup_open/* EXT_DEMIURGE: */ || !enabled /* EXT_DEMIURGE: */)
        return false;
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items/* EXT_DEMIURGE: */, bool enabled /* = true */, bool indeterminate /* = false */ /* EXT_DEMIURGE: */)
{
    ImGuiContext& g = *GImGui;
    ...
    if (!BeginCombo(label, preview_value, ImGuiComboFlags_None/* EXT_DEMIURGE: */, enabled, indeterminate /* EXT_DEMIURGE: */))
        return false;
...
bool ImGui::Combo(const char* label, int* current_item, const char* const items[], int items_count, int height_in_items/* EXT_DEMIURGE: */, bool enabled /* = true */, bool indeterminate /* = false */ /* EXT_DEMIURGE: */)
{
    const bool value_changed = Combo(label, current_item, Items_ArrayGetter, (void*)items, items_count, height_in_items/* EXT_DEMIURGE: */, enabled, indeterminate /* EXT_DEMIURGE: */);
    return value_changed;
}

// Combo box helper allowing to pass all items in a single string literal holding multiple zero-terminated items "item1\0item2\0"
bool ImGui::Combo(const char* label, int* current_item, const char* items_separated_by_zeros, int height_in_items/* EXT_DEMIURGE: */, bool enabled /* = true */, bool indeterminate /* = false */ /* EXT_DEMIURGE: */)
{
    int items_count = 0;
    ...
    bool value_changed = Combo(label, current_item, Items_SingleStringGetter, (void*)items_separated_by_zeros, items_count, height_in_items/* EXT_DEMIURGE: */, enabled, indeterminate /* EXT_DEMIURGE: */);
    return value_changed;
}
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags/* EXT_DEMIURGE: */, ImGuiInputTextFlags extra_flags /* = 0 */ /* EXT_DEMIURGE: */)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const bool temp_input_allowed = (flags & ImGuiSliderFlags_NoInput) == 0/* EXT_DEMIURGE: */ && (extra_flags & ImGuiInputTextFlags_ReadOnly) == 0/* EXT_DEMIURGE: */;
    ItemSize(total_bb, style.FramePadding.y);
    ...
    if (extra_flags & ImGuiInputTextFlags_Indeterminate) { // EXT_DEMIURGE:
        const ImVec2 center = frame_bb.GetCenter(); // EXT_DEMIURGE:
        const float fLeft = frame_bb.Min.x + style.FramePadding.x; // EXT_DEMIURGE:
        const float fRight = ImMin( // EXT_DEMIURGE:
            frame_bb.Max.x - style.FramePadding.x, // EXT_DEMIURGE:
            frame_bb.Min.x + style.FramePadding.x + g.FontSize); // EXT_DEMIURGE:
...
bool ImGui::DragFloat(const char* label, float* v, float v_speed, float v_min, float v_max, const char* format, ImGuiSliderFlags flags/* EXT_DEMIURGE: */, ImGuiInputTextFlags extra_flags /* = 0 */ /* EXT_DEMIURGE: */)
{
    return DragScalar(label, ImGuiDataType_Float, v, v_speed, &v_min, &v_max, format, flags/* EXT_DEMIURGE: */, extra_flags /* EXT_DEMIURGE: */);
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const bool init_make_active = (/* EXT_DEMIURGE: */ (ImGuiInputTextFlags_SetFocus & flags) || /* EXT_DEMIURGE: */ user_clicked || user_scroll_finish || input_requested_by_nav || input_requested_by_tabbing);
    ...
    else if (flags & ImGuiInputTextFlags_Indeterminate) // EXT_DEMIURGE:
    { // EXT_DEMIURGE:
        const ImVec2 center = frame_bb.GetCenter(); // EXT_DEMIURGE:
        const float left = frame_bb.Min.x + style.FramePadding.x; // EXT_DEMIURGE:
        const float right = ImMin( // EXT_DEMIURGE:
            frame_bb.Max.x - style.FramePadding.x, // EXT_DEMIURGE:
            frame_bb.Min.x + style.FramePadding.x + g.FontSize); // EXT_DEMIURGE:

        draw_window->DrawList->AddLine( // EXT_DEMIURGE:
            ImVec2(left, center.y), ImVec2(right, center.y), GetColorU32(ImGuiCol_Text)); // EXT_DEMIURGE:
    } // EXT_DEMIURGE:
    else
    {
...
// EXT_DEMIURGE:
static ImGuiKey GetAcceleratorKey(const char* label)
{
    const char* pos = strchr(label, '&');
    if (NULL == pos)
    {
    ...
    {
        const char* s = (pos + 1);
        if (*s >= 'a' && *s <= 'z') { return (ImGuiKey)(ImGuiKey_A + (*s - 'a')); }
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    // EXT_DEMIURGE:
    ImGuiKey const accelerator_key = GetAcceleratorKey(label);
    ImGuiTextMarkupMode const ampersand_as_accelerator = (g.IO.KeyAlt && !g.IO.WantCaptureKeyboard && !g.IO.WantTextInput)
        ? ImGuiTextMarkupMode_AmpersandAsUnderline
        : ImGuiTextMarkupMode_AmpersandStrip;
    // /EXT_DEMIURGE:

    ImVec2 label_size = CalcTextSize(label, NULL, true/* EXT_DEMIURGE: */, -1.0f, ampersand_as_accelerator /* EXT_DEMIURGE: */);
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        RenderText(text_pos, label/* EXT_DEMIURGE: */, 0, true, ampersand_as_accelerator /* EXT_DEMIURGE: */);
        PopStyleVar();
    ...
    /* EXT_DEMIURGE: const*/ bool hovered = (g.HoveredId == id) && enabled && !g.NavDisableMouseHover;

    // EXT_DEMIURGE:
    if (!g.AcceleratedThisFrame &&
        !g.IO.WantCaptureKeyboard &&
        !g.IO.WantTextInput &&
        !menu_is_open &&
        enabled &&
        accelerator_key != ImGuiKey_None)
    {
        if (g.IO.KeyAlt && IsKeyPressed(accelerator_key))
        {
            hovered = true;
            pressed = true;
            g.AcceleratedThisFrame = true;
        }
    }
    // /EXT_DEMIURGE:
    ...
    else
    {
        // Menu bar
        // EXT_DEMIURGE:
        if (menu_is_open && IsKeyPressed(ImGuiKey_Escape) && menuset_is_open)
        {
            want_close = true;
            want_open = menu_is_open = false;
        }
        else if (menu_is_open && accelerator_key != ImGuiKey_None && g.IO.KeyAlt && IsKeyPressed(accelerator_key) && menuset_is_open)
        {
            want_close = true;
            want_open = menu_is_open = false;
        }
...
bool ImGui::MenuItemEx(const char* label, const char* icon, const char* shortcut, bool selected, bool enabled)
{
    ...
    ImGuiTextMarkupMode const ampersand_as_accelerator = (g.IO.KeyAlt && !g.IO.WantCaptureKeyboard && !g.IO.WantTextInput)
        ...
        : ImGuiTextMarkupMode_AmpersandStrip;
    // /EXT_DEMIURGE:
    ImVec2 label_size = CalcTextSize(label, NULL, true/* EXT_DEMIURGE: */, -1.0f, accelerator_key /* EXT_DEMIURGE: */);
    ...
    else
    {
        ...
        RenderText(pos + ImVec2(offsets->OffsetLabel, 0.0f), label/* EXT_DEMIURGE: */, 0, true, ampersand_as_accelerator /* EXT_DEMIURGE: */);
        if (icon_w > 0.0f)
            RenderText(pos + ImVec2(offsets->OffsetIcon, 0.0f), icon);
        if (shortcut_w > 0.0f)
        {
