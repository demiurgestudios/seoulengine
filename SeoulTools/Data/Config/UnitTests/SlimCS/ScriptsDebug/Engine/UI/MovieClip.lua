...
function MovieClip:SetGameStateValue(k, v)
	...
	name = self:GetNative():GetMovieTypeName()
...
function MovieClip:IsInSceneGraph()

	local rootMovieClip = self:GetRootMovieClip()
	if nil == rootMovieClip then

		return false
	end

	local currentMovieClip = self
	if currentMovieClip == rootMovieClip then
	...
	while true do
		...
		if currentMovieClip:GetParent() == rootMovieClip then

			return true

		elseif currentMovieClip:GetParent() == nil then
