...
function RootMovieClip:OnHotLoadBegin()

	if nil == self.m_tHotLoadTargets then

		return
	end

	for inst, _ in pairs(self.m_tHotLoadTargets) do

		inst:OnHotLoadBeginHandler()
	end
end

function RootMovieClip:OnHotLoadEnd()
...
function RootMovieClip:RegisterForHotLoad(inst)

	if nil == self.m_tHotLoadTargets then

		self.m_tHotLoadTargets = {}
	end

	self.m_tHotLoadTargets[inst] = inst
end
...
function RootMovieClip:Constructor() self.m_aRootEveryTickTasks = {}; self.m_aRootTasks = {}; self.m_tPendingRequests = {}; self.m_bRootIsInAnimOut = false;
	...
	DynamicGameStateData.RegisterRootMovieClip(self, self:GetNative():GetMovieTypeName())
...
function RootMovieClip:Destructor()
	...
	DynamicGameStateData.UnregisterRootMovieClip(self)
