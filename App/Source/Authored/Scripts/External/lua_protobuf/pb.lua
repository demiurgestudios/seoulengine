...
local typeinfo = require "External/lua_protobuf/pb_typeinfo"
...
local function field_type(field)
   local realtype = typeinfo
   for _,v in ipairs(field.type_name) do
      realtype = realtype[v]
...
function pb.clearbuffers()
   for k, _ in pairs(buffer_pool) do
      k:reset()
      buffer_pool[k] = nil
   end
end
...
local function decode_field(t, dec, wiretype, _, field)
   local value
...
function encode(buff, t, ptype)
   ...
   for k,v in pairs(t) do
      ...
      if tag then
         table.insert(a, { tag = tag, v = v })
      end
   end

   table.sort(a, function(a, b) return a.tag < b.tag end)
   
   for _,e in ipairs(a) do
      encode_field(buff, e.tag, e.v, ptype)
   end
end
   ...
   if enum.value then
      ...
      for _, v in ipairs(enum.value) do
         ...
         if v.options and v.options.deprecated then
            ...
            -- EXT_DEMIURGE: Was deprecated_names[k], which is the left-over iteration
            -- var from the previous loop. Guessing v.name is the intended key here.
            deprecated_names[v.name] = true
         end
      end
   end
...
local function load_message(pkg, msg)
   ...
   if msg.field then
      ...
      -- load new fields
      for _, v in ipairs(msg.field) do
         load_field(t, v)
   ...
   if msg.nested_type then
      for _, v in ipairs(msg.nested_type) do
         load_message(t, v)
      end
   end
   if msg.enum_type then
      for _, v in ipairs(msg.enum_type) do
         load_enum(t, v)
      end
   end
   if msg.extension then
      for _, v in ipairs(msg.extension) do
         load_extension(v)
      end
   end
...
local function load_file(file)
   -- EXT_DEMIURGE: Package is optional, but pb.lua fails badly without one
   if not file.package then
      print("Protobuf files must have a package defined")
   end
   local pkg = make_package(file.package)
   if file.message_type then
      for _, v in ipairs(file.message_type) do
         load_message(pkg, v)
      end
   end
   if file.enum_type then
      for _, v in ipairs(file.enum_type) do
         load_enum(pkg, v)
      end
   end
   if file.extension then
      for _, v in ipairs(file.extension) do
         load_extension(v)
      end
   end
end
...
function pb.to_map(tin)
    local tout = {}
    for k,v in pairs(tin) do
        table.insert(tout, { key=k, value=v })
    end
	return tout
end
-- /EXT_DEMIURGE:
