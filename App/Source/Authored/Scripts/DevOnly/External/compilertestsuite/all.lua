...
local _port = _port -- EXT_DEMIURGE: _port = false      -- true to avoid non-portable tests
local _no32 = _no32 -- EXT_DEMIURGE: _no32 = false      -- true to avoid tests that assume 32 bits
_nomsg = false     -- true to avoid messages about tests not performed
_noposix = false   -- false assumes LUA_USE_POSIX
local _nolonglong = _nolonglong -- EXT_DEMIURGE: _nolonglong = false  -- false assumes LUA_USE_LONGLONG
local _noformatA = _noformatA -- EXT_DEMIURGE: _noformatA = false   -- false assumes LUA_USE_AFORMAT
local _notesttime = _notesttime -- EXT_DEMIURGE:
...
do
  ...
  mt.__gc = function (o)
    -- EXT_DEMIURGE: stderr:write'.'    -- mark progress
...
dofile('math.lua')
dofile('sort.lua')
-- EXT_DEMIURGE: dofile('bitwise.lua')
-- EXT_DEMIURGE: assert(dofile('verybig.lua') == 10); collectgarbage()
-- EXT_DEMIURGE: dofile('files.lua')
...
-- EXT_DEMIURGE: if not usertests then
if not _notesttime then
