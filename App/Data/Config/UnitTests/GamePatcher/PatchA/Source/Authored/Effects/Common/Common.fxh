...
#	define SEOUL_PS(name) SetPixelShader(CompileShader(ps_4_0, name()))
#	define SEOUL_PS_NULL SetPixelShader(0);
#	define SEOUL_VS(name) SetVertexShader(CompileShader(vs_4_0, name()))
...
#	define SEOUL_PS(name) PixelShader = compile ps_3_0 name()
#	define SEOUL_PS_NULL PixelShader = null;
#	define SEOUL_VS(name) VertexShader = compile vs_3_0 name()
...
#	define SEOUL_EXT_BLEND_MODE(d3d9src, d3d9dst, src, dst) \
		...
		// when translating this shared into GLSL, which should be exactly
		// the same result.
		//
